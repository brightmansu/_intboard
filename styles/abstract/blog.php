<?
function blog_params($catselect,$levelselect,$fdata,$fcontainer,$langselect) { ?>
...
function blog_list_start($pages,$owner) {
...
<? if ($GLOBALS['inuserlevel']>=$GLOBALS['inforum']['f__ltopic']) { ?><div style="float: right"><a href="<?=build_url($GLOBALS['inforum'],'a=add_news');?>"><?=MSG_blog_add;?></a></div><? } ?>
<br><?=MSG_blog_owner;?>: <?=user_out($owner[0],$owner[1]);?> <?
...
<table class="innertable posttable" width="100%" cellspacing=1><tr><td class="tablehead">
<?=$GLOBALS['inforum']['f_title'];?><div style="text-align:right"><?=$pages;?></div>
<? }

function blog_list_entry(&$ndata) {
?><tr><td class=="postentry">
<?=long_date_out($ndata['p__time']);?> - <b><a href="<?=build_url($ndata);?>"><?=$ndata['t_title'];?></a></b><br><?
if ($ndata['p_attach']) {
...
<?=textout($ndata['p_text'],$ndata['p__html'],$ndata['p__bcode'],$ndata['p__smiles'],$ndata['t_id'],$ndata['n_id']);?><br>
<div class="descr" style="text-align: right; clear: both"><a href="<?=build_url($ndata);?>"><?=format_word($ndata['pcount'],MSG_comment1,MSG_comment2,MSG_comment3);?></a> | <a href="<?=build_url($ndata);?>"><?=MSG_blog_comment;?></a></div>
<? if ($GLOBALS['inuserlevel']>=$GLOBALS['inforum']['f_lmoderate']) { ?>
<a class="descr" href="<?=build_url($ndata,'m=blog&amp;a=edit_news');?>"><?=MSG_blog_edit;?></a>
<? } ?>
<? }

function blog_list_noentries() { ?>
<tr><td><?=MSG_blog_nonews;?>
<? }

function blog_list_end($inforum,$autosub) { ?>
</table><?
...
<br><div class="modlinks"><a href="<?=build_url($GLOBALS['inforum'],'a=add_news');?>"><?=MSG_blog_add;?></a>
<?
if ($GLOBALS['inuserlevel']>=$GLOBALS['inforum']['f_lmoderate']) {
...
function blog_owner($owner) { ?>
<?=MSG_blog_owner;?>: <?=user_out($owner[0],$owner[1]);?><? 
if ($owner[2]!=1 && $GLOBALS['inuserid']!=$owner[1]) { 
  ?>(<a onClick="return confirm('<?=MSG_p_friend_warn1.' '.$owner[0].' '.MSG_p_friend_warn2;?>')" href="index.php?m=addrbook&amp;a=do_friend&amp;uid=<?=$owner[0];?>"><?=MSG_p_friend;?></a>)<? 
}
}

function blog_edit_form(&$tdata,&$pdata,$newaction,$msg) {
...
<? } ?>
<? common_post($pdata,MSG_blog_text);?>
<? if ($GLOBALS['inuserlevel']>=$inforum['f_lattach']) { ?>
<tr><td><?=MSG_blog_attach;?>:
...
function blog_print_form(&$tdata,&$pdata) { ?>
